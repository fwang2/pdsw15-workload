...
Figure~\ref{fig:ddnpeakBW} (a) and Figure~\ref{fig:ddnpeakBW}(b), shows the
...
believe the middleware libraries such as ADIOS~\cite{adios} are helping to
...
Spider 2 system was rated for +1 TB/s at the file system level, if exercised
with well-formed, large, sequential I/O on a quiet system. However, under
...
sizes). Therefore, the Spider 2 system was expected to perform roughly at 250
GB/s under bursty production workloads~\cite{bestpractices}.


As can be seen from the Figure~\ref{fig:bwUsage}, for more than 95\% of the the
...
of I/O less than 5\% of their compute time allocations. This suggests that our scientific workloads can benefit from a burst buffer storage layer to absorb these bursty I/O patterns.
