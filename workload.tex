...
Figure~\ref{fig:ddnpeakBW} (a) and Figure~\ref{fig:ddnpeakBW}(b), shows the peak
read and write bandwidth observed at the DDN RAID controllers for Spider 1 and Spider 2, respectively.

On Spider 1, the peak read bandwidth observed was 90\% of the maximum on the RAID
controllers. The maximum aggreagatec( combined read and write) bandwidth of 3
GB/s for the controller on Spider 1, and 36GB/s for the controllers on Spider 2. 
The write peaked at 50\% for Spider 1. On Spider 2 the reads peaked at
80% of the capacity. However, on Spider 2 the controllers peaked at 70% of their maximum
for writes. 

This is could be atrributed to the gemini interconnect, increased I/O ROUTERS.  
...
Figure~\ref{fig:bwUsage}, shows the aggregate bandwidth, sum of read and write bandwidth,
...
We see that more for more than 95% of the storage uptime the
bandwidth was less than 50 GB/s. Only around 4% of the time the bandwidth was
over 50 GB/s. For less than 1 % of the time the bandwidth peaked over 250GB/s. 

This is expected, as typically scientific applications are compute intensive,
with I/O less than 5% of their compute time and occur in bursts. Also, say
we sustain a bandwidth of 250GB/s for 10 minutes in a day, which is 0.69%, we
will be generating 146TB's of data.

From Figure~\ref{fig:storageUsage}, we see the storage space utilization on the system, with more 50% of the capacity being used. 
