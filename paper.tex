%\documentclass[conference]{IEEEtran}
%\documentclass{acm_proc_article-sp}
\documentclass{sig-alternate-05-2015}
\usepackage{subfigure}          % subfigures...
\usepackage{color}
\usepackage{algorithm}      % for description of algorithms...
\usepackage{algorithmic}
\usepackage{graphicx}           % to include eps figures...
\usepackage{epstopdf}
\usepackage{indentfirst}
\usepackage{verbatim}
\usepackage{amsmath}
\usepackage{url}
%\usepackage[dvipdfm]{graphicx}
\usepackage{tabularx,ragged2e,booktabs,caption}


\newtheorem{definition}{Definition}
\graphicspath{{Fig/}}

\definecolor{cpur}{rgb}{0.50,0.00,0.25}
\definecolor{red}{rgb}{1.00,0.00,0.00}
\definecolor{darkgreen}{rgb}{0.078,0.667,0.016}
\newcommand{\cm}{\textcolor{magenta}}
\newcommand{\cb}{\textcolor{blue}}
\newcommand{\cred}{\textcolor{red}}
\newcommand{\cy}{\textcolor{cyan}}
\newcommand{\cg}{\textcolor{darkgreen}}

%\partopsep 2pt

%\setlength{\parindent}{1.5em}
%\sloppy



%\newfont{\mycrnotice}{ptmr8t at 7pt}
%\newfont{\myconfname}{ptmri8t at 7pt}
%\let\crnotice\mycrnotice%
%\let\confname\myconfname%

%\begin{document}

%\permission{ \copyright 2015 Association for Computing Machinery. ACM acknowledges that this contribution was authored or co-authored by an employee, contractor or affiliate of the United States government. As such, the United States Government retains a nonexclusive, royalty-free right to publish or reproduce this article, or to allow others to do so, for Government purposes only.}
%\conferenceinfo{PDSW2015,}{ November 15-20, 2015, Austin, TX, USA}
%\copyrightetc{Copyright 2015 ACM \the\acmcopyr}
%\crdata{ISBN 978-1-4503-4008-3/15/11…$15.00.\\
%DOI: http://dx.doi.org/10.1145/2834976.2834985}


%==============================================================================
\begin{document}

%\setcopyright{acmcopyright}
\setcopyright{usgov}

\doi{http://dx.doi.org/10.1145/2834976.2834985}
\isbn{978-1-4503-4008-3/15/11}
\conferenceinfo{PDSW2015,}{ November 15-20, 2015, Austin, TX, USA}
\CopyrightYear{2015}
\acmPrice{\$15.00}

\title{Comparative I/O Workload Characterization of\\
Two Leadership Class Storage Clusters}
%
% You need the command \numberofauthors to handle the 'placement
% and alignment' of the authors beneath the title.
%
% For aesthetic reasons, we recommend 'three authors at a time'
% i.e. three 'name/affiliation blocks' be placed beneath the title.
%
% NOTE: You are NOT restricted in how many 'rows' of
% "name/affiliations" may appear. We just ask that you restrict
% the number of 'columns' to three.
%
% Because of the available 'opening page real-estate'
% we ask you to refrain from putting more than six authors
% (two rows with three columns) beneath the article title.
% More than six mmekakes the first-page appear very cluttered indeed.
%
% Use the \alignauthor commands to handle the names
% and affiliations for an 'aesthetic maximum' of six authors.
% Add names, affiliations, addresses for
% the seventh etc. author(s) as the argument for the
% \additionalauthors command.
% These 'additional authors' will be output/set for you
% without further effort on your part as the last section in
% the body of your article BEFORE References or any Appendices.

%\numberofauthors{8} %  in this sample file, there are a *total*
% of EIGHT authors. SIX appear on the 'first-page' (for formatting
% reasons) and the remaining two appear in the \additionalauthors section.
%

\author{ 
\alignauthor
Raghul Gunasekaran, Sarp Oral, Jason Hill, Ross Miller, Feiyi Wang, Dustin Leverman\\
\vspace{0.05in}
\affaddr{\large Oak Ridge Leadership Compute Facility, Oak Ridge National Laboratory}\\
\vspace{0.05in}
\email{\large \{gunasekaranr,oralhs,hilljj,rgmiller,fwang2,leverman\}@ornl.gov}
}


\maketitle
\input abstract
\input intro
\input overview
\input workload 
%\input discussion
%\input relatedwork
\input conclusion

% use section* for acknowledgement
\section*{Acknowledgment}
This work was supported by the Oak Ridge Leadership Computing Facility at the Oak Ridge National Laboratory, which is managed by UT Battelle, LLC for the U.S. DOE (under the contract No. DE-AC05-00OR22725).

%\cleardoublepage
%\clearpage
\nocite{reference}
\bibliographystyle{abbrv}
\bibliography{refs}
\end{document}

